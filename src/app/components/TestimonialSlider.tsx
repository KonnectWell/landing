  ...
  return (
    <Swiper loop>
      {slideImages.map((image, idx) => {
        const testimonials = [
          {
            text: "KonnectWell found me the perfect advisor for my retirement planning needs!",
            rating: 5
          },
          {
            text: "The matching process was incredibly accurate and personalized.",
            rating: 5
          }
        ]
                  ...
                  <div>
                    ...
                    <p className="mt-2">
                      <span className="flex items-center gap-1 text-base text-yellow-300">
                        {Array.from(new Array(testimonials[idx].rating)).map((_val, idx) => {
                          return (
                            <LuStar
                              key={idx}
                              data-lucide="star"
